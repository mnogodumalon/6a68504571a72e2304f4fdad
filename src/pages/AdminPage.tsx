...
const KURSE_FIELDS = [
  ...
  { key: 'kurstyp', label: 'Kurstyp', type: 'lookup/select', options: [{ key: 'vinyasa', label: 'Vinyasa Yoga' }, { key: 'yin', label: 'Yin Yoga' }, { key: 'restorative', label: 'Restorative Yoga' }, { key: 'ashtanga', label: 'Ashtanga Yoga' }, { key: 'kundalini', label: 'Kundalini Yoga' }, { key: 'power', label: 'Power Yoga' }, { key: 'meditation', label: 'Meditation' }, { key: 'sonstiges', label: 'Sonstiges' }, { key: 'hatha', label: 'Hatha Yoga' }] },
  { key: 'schwierigkeitsgrad', label: 'Schwierigkeitsgrad', type: 'lookup/radio', options: [{ key: 'anfaenger', label: 'Anfänger' }, { key: 'mittelstufe', label: 'Mittelstufe' }, { key: 'fortgeschritten', label: 'Fortgeschritten' }, { key: 'alle_levels', label: 'Alle Levels' }] },
  { key: 'dauer_minuten', label: 'Dauer (Minuten)', type: 'number' },
  { key: 'kursleiter', label: 'Kursleiter/in', type: 'string/text' },
  { key: 'max_teilnehmer', label: 'Maximale Teilnehmerzahl', type: 'number' },
  { key: 'beschreibung', label: 'Kursbeschreibung', type: 'string/textarea' },
  { key: 'aktiv', label: 'Kurs aktiv', type: 'bool' },
];
const TERMINE_FIELDS = [
  ...
  { key: 'status', label: 'Status', type: 'lookup/select', options: [{ key: 'geplant', label: 'Geplant' }, { key: 'abgesagt', label: 'Abgesagt' }, { key: 'durchgefuehrt', label: 'Durchgeführt' }] },
  { key: 'angemeldete_teilnehmer', label: 'Anzahl Anmeldungen', type: 'number' },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
];
const ANMELDUNGEN_FIELDS = [
  { key: 'termin', label: 'Termin', type: 'applookup/select', targetEntity: 'termine', targetAppId: 'TERMINE', displayField: 'raum' },
  ...
  { key: 'anmeldedatum', label: 'Anmeldedatum und -uhrzeit', type: 'date/datetimeminute' },
  { key: 'anmeldequelle', label: 'Anmeldequelle', type: 'lookup/radio', options: [{ key: 'online', label: 'Online' }, { key: 'telefonisch', label: 'Telefonisch' }, { key: 'vor_ort', label: 'Vor Ort' }] },
  { key: 'status_anmeldung', label: 'Status der Anmeldung', type: 'lookup/select', options: [{ key: 'neu', label: 'Neu / Ausstehend' }, { key: 'bestaetigt', label: 'Bestätigt' }, { key: 'abgesagt', label: 'Abgesagt' }, { key: 'warteliste', label: 'Warteliste' }] },
  { key: 'notizen_anmeldung', label: 'Notizen', type: 'string/textarea' },
];
...
export default function AdminPage() {
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'termine':
        ...
        break;
      case 'anmeldungen':
        lists.termineList = (data as any).termine ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'termine' && fieldKey === 'kurs') {
      const match = (lists.kurseList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.kursname ?? '—';
    }
    if (entity === 'anmeldungen' && fieldKey === 'termin') {
      const match = (lists.termineList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.raum ?? '—';
    }
    return String(url);
  }, [getLookupLists]);
    ...
    >
      ...
      {(createEntity === 'anmeldungen' || dialogState?.entity === 'anmeldungen') && (
        <AnmeldungenDialog
          ...
          termineList={(data as any).termine ?? []}
