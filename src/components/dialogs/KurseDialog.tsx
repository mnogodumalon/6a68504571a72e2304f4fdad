...
import type { Kurse, LookupValue } from '@/types/app';
...
export type KurseDialogDefaults = Omit<Kurse['fields'], 'kurstyp' | 'schwierigkeitsgrad'> & {
    kurstyp?: LookupValue | string;
    schwierigkeitsgrad?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  kurstyp: LOOKUP_OPTIONS['kurse']?.['kurstyp'] ?? [],
  schwierigkeitsgrad: LOOKUP_OPTIONS['kurse']?.['schwierigkeitsgrad'] ?? [],
};
...
export function KurseDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: KurseDialogProps) {
  ...
  const REQUIRED_FIELDS = ['kursname', 'kurstyp', 'kursleiter', 'max_teilnehmer'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "kursname": string | null, // Kursname\n  "kurstyp": LookupValue | null, // Kurstyp (select one key: "vinyasa" | "yin" | "restorative" | "ashtanga" | "kundalini" | "power" | "meditation" | "sonstiges" | "hatha") mapping: vinyasa=Vinyasa Yoga, yin=Yin Yoga, restorative=Restorative Yoga, ashtanga=Ashtanga Yoga, kundalini=Kundalini Yoga, power=Power Yoga, meditation=Meditation, sonstiges=Sonstiges, hatha=Hatha Yoga\n  "schwierigkeitsgrad": LookupValue | null, // Schwierigkeitsgrad (select one key: "anfaenger" | "mittelstufe" | "fortgeschritten" | "alle_levels") mapping: anfaenger=Anfänger, mittelstufe=Mittelstufe, fortgeschritten=Fortgeschritten, alle_levels=Alle Levels\n  "dauer_minuten": number | null, // Dauer (Minuten)\n  "kursleiter": string | null, // Kursleiter/in\n  "max_teilnehmer": number | null, // Maximale Teilnehmerzahl\n  "beschreibung": string | null, // Kursbeschreibung\n  "aktiv": boolean | null, // Kurs aktiv\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'kursname': (
      <div key="kursname" className="space-y-1.5">
        ...
        <Input
          id="kursname"
          placeholder="z. B. Vinyasa für Anfänger"
    ...
    'kurstyp': (
      <div key="kurstyp" className="space-y-1.5">
        <Label htmlFor="kurstyp">Kurstyp <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Select
          value={lookupKey(fields.kurstyp) ?? ''}
          onValueChange={v => setFields(f => ({ ...f, kurstyp: v === 'none' ? undefined : v as any }))}
        >
          <SelectTrigger id="kurstyp" className="max-sm:h-11"><SelectValue placeholder="Wähle einen Kurstyp" /></SelectTrigger>
          <SelectContent>
    ...
    'schwierigkeitsgrad': (
      <div key="schwierigkeitsgrad" className="space-y-1.5">
        <Label htmlFor="schwierigkeitsgrad">Schwierigkeitsgrad</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.schwierigkeitsgrad) === 'anfaenger'}
            onClick={() => setFields(f => ({ ...f, schwierigkeitsgrad: (lookupKey(f.schwierigkeitsgrad) === 'anfaenger' ? undefined : 'anfaenger') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.schwierigkeitsgrad) === 'anfaenger'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.schwierigkeitsgrad) === 'mittelstufe'}
            onClick={() => setFields(f => ({ ...f, schwierigkeitsgrad: (lookupKey(f.schwierigkeitsgrad) === 'mittelstufe' ? undefined : 'mittelstufe') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.schwierigkeitsgrad) === 'mittelstufe'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.schwierigkeitsgrad) === 'fortgeschritten'}
            onClick={() => setFields(f => ({ ...f, schwierigkeitsgrad: (lookupKey(f.schwierigkeitsgrad) === 'fortgeschritten' ? undefined : 'fortgeschritten') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.schwierigkeitsgrad) === 'fortgeschritten'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.schwierigkeitsgrad) === 'alle_levels'}
            onClick={() => setFields(f => ({ ...f, schwierigkeitsgrad: (lookupKey(f.schwierigkeitsgrad) === 'alle_levels' ? undefined : 'alle_levels') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.schwierigkeitsgrad) === 'alle_levels'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'kursleiter': (
      <div key="kursleiter" className="space-y-1.5">
        <Label htmlFor="kursleiter">Kursleiter/in <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="kursleiter"
          placeholder="Name des Kursleiters"
          value={fields.kursleiter ?? ''}
          onChange={e => setFields(f => ({ ...f, kursleiter: e.target.value }))}
          required
        />
    ...
    'max_teilnehmer': (
      <div key="max_teilnehmer" className="space-y-1.5">
        <Label htmlFor="max_teilnehmer">Maximale Teilnehmerzahl <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="max_teilnehmer"
          type="number"
          step="any"
    ...
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        ...
        <Textarea
          id="beschreibung"
          placeholder="Zielgruppe, Inhalte, Besonderheiten..."
    ...
    'aktiv': (
      <div key="aktiv" className="space-y-1.5">
        <Label htmlFor="aktiv">Kurs aktiv</Label>
        <div className="flex items-center gap-2 pt-1">
          ...
          />
          <Label htmlFor="aktiv" className="font-normal">Kurs aktiv</Label>
        </div>
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"kursname": "Kursname", "kurstyp": "Kurstyp", "schwierigkeitsgrad": "Schwierigkeitsgrad", "dauer_minuten": "Dauer (Minuten)", "kursleiter": "Kursleiter/in", "max_teilnehmer": "Maximale Teilnehmerzahl", "beschreibung": "Kursbeschreibung", "aktiv": "Kurs aktiv"};
