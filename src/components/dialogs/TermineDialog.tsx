 ...
 * kurseList (full hook array — resolves the Kurse applookup),
...
import type { Termine, Kurse, LookupValue } from '@/types/app';
...
export type TermineDialogDefaults = Omit<Termine['fields'], 'status'> & {
...
export function TermineDialog({ open, onClose, onSubmit, defaultValues, recordId, kurseList, enablePhotoScan = true, enablePhotoLocation = true }: TermineDialogProps) {
  ...
  // Inline-Create state for "Kurse" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraKurse` list, and select it in
  // the originating Combobox via the captured `createKurseField`.
  const [createKurseOpen, setCreateKurseOpen] = useState(false);
  const [createKurseInitial, setCreateKurseInitial] = useState('');
  const [createKurseField, setCreateKurseField] = useState<string>('');
  const [extraKurse, setExtraKurse] = useState< Kurse[]>([]);
  const kurseListAll = useMemo(
    () => [...kurseList, ...extraKurse],
    [kurseList, extraKurse],
  );
  function openCreateKurse(fieldKey: string, q: string) {
    setCreateKurseField(fieldKey);
    setCreateKurseInitial(q);
    setCreateKurseOpen(true);
  }
  const [showErrors, setShowErrors] = useState(false);
  const REQUIRED_FIELDS = ['kurs', 'datum_uhrzeit', 'status'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "kurs": string | null, // Display name from Kurse (see <available-records>)\n  "datum_uhrzeit": string | null, // YYYY-MM-DDTHH:MM\n  "raum": string | null, // Raum / Ort\n  "status": LookupValue | null, // Status (select one key: "geplant" | "abgesagt" | "durchgefuehrt") mapping: geplant=Geplant, abgesagt=Abgesagt, durchgefuehrt=Durchgeführt\n  "angemeldete_teilnehmer": number | null, // Anzahl Anmeldungen\n  "notizen": string | null, // Notizen\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'kurs': (
      <div key="kurs" className="space-y-1.5">
        ...
        <Combobox
          ...
          onChange={id => setFields(f => ({ ...f, kurs: id ? createRecordUrl(APP_IDS.KURSE, id) : undefined }))}
          searchPlaceholder="Suchen…"
          emptyText="Kein Treffer"
          onCreateNew={(q) => openCreateKurse("kurs", q)}
          createLabel="Neu in Kurse"
        />
    ...
    'raum': (
      <div key="raum" className="space-y-1.5">
        ...
        <Input
          id="raum"
          placeholder="z. B. Hauptsaal, Studio West"
          value={fields.raum ?? ''}
      ...
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'durchgefuehrt'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'durchgefuehrt' ? undefined : 'durchgefuehrt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'durchgefuehrt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
    ...
    'angemeldete_teilnehmer': (
      <div key="angemeldete_teilnehmer" className="space-y-1.5">
        <Label htmlFor="angemeldete_teilnehmer">Anzahl Anmeldungen</Label>
        <Input
          id="angemeldete_teilnehmer"
          type="number"
          step="any"
  ...
  const FIELD_LABELS: Record<string, string> = {"kurs": "Kurs", "datum_uhrzeit": "Datum und Uhrzeit", "raum": "Raum / Ort", "status": "Status", "angemeldete_teilnehmer": "Anzahl Anmeldungen", "notizen": "Notizen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"kurs": {"kursname": "Kursname", "kurstyp": "Kurstyp", "schwierigkeitsgrad": "Schwierigkeitsgrad", "dauer_minuten": "Dauer (Minuten)", "kursleiter": "Kursleiter/in", "max_teilnehmer": "Maximale Teilnehmerzahl", "beschreibung": "Kursbeschreibung", "aktiv": "Kurs aktiv"}};
  ...
  return (
    ...
    {createKurseOpen && (
      <KurseDialog
        open={createKurseOpen}
        onClose={() => setCreateKurseOpen(false)}
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createKurseEntry(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as Kurse;
            setExtraKurse(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.KURSE, result.id);
            setFields(prev => ({ ...prev, [createKurseField]: url } as any));
          }
          setCreateKurseOpen(false);
        }}
        defaultValues={createKurseInitial
          ? ({ kursname: createKurseInitial } as any)
          : undefined}
      />
    )}
    </>
