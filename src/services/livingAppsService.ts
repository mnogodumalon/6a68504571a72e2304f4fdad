...
import type { Kurse, Termine, Anmeldungen, CreateKurse, CreateTermine, CreateAnmeldungen } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getTermineEntry(id: string): Promise<Termine | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Termine;
    return enrichLookupFields([record], 'termine')[0];
  }
  static async createTermineEntry(fields: CreateTermine) {
    return callApi('POST', `/apps/${APP_IDS.TERMINE}/records`, { fields: cleanFieldsForApi(fields as any, 'termine') });
  }
  static async updateTermineEntry(id: string, fields: Partial<CreateTermine>) {
    return callApi('PATCH', `/apps/${APP_IDS.TERMINE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'termine') });
  }
